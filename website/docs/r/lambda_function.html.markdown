...
To give an external source (like a CloudWatch Event Rule, SNS, or S3) permission to access the Lambda function, use the [`aws_lambda_permission`] resource (lambda_permission.html). See [Lambda Permission Model][4] for more details.
